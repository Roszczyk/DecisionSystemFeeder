from sensor import Sensor, TruthTable
...
class ScalarSensor(Sensor):
    def __init__(self, name : str, truth_table : TruthTable, precision : float,
                 measure_function : function, uncertainty_logic : function,
                 labels_thresholds : list[LabelThreshold]):
        super().__init__(name, truth_table, precision, measure_function)
    ...
    def gauss_probability(self, value, uncertainty, label_threshold : LabelThreshold):
        ...
        p = norm.cdf(label_threshold.upper, loc=value, scale=uncertainty) - norm.cdf(label_threshold.lower, loc=value, scale=uncertainty)
        return float(p)
...
def dummy_run():
    ...
    sensor = ScalarSensor("dummy", None, None, measure_dummy, uncertainty_func_dummy, labels)
